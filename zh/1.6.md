...
- Go modules版本依赖管理特性将从Go SDK 1.13开始大规模推荐使用。`GOPATH`环境变量的地位将减弱甚至丧失。`GOBIN`环境变量的地位将提升，因为`go install`命令仍需要一个路径来存储生成的二进制可执行文件。另外，伴随Go modules而生的`GOPROXY`环境变量的设置格式得到了增强。我们可以使用下面的格式来指定多个代理：
